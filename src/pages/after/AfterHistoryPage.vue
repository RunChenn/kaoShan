      ...
      >
        ...
        <div class="hcard-header" @click="toggleExpand(record.id)">
          <div class="hcard-icon-ring" :class="`diff-${record.difficulty}`">
            <q-icon name="landscape" size="18px" :color="difficultyColor(record.difficulty)" />
          </div>
          <div class="col">
        ...
        <transition name="expand-fade">
          <div v-if="expanded === record.id" class="hcard-detail">
            ...
            <div class="stat-row q-mt-sm q-mb-md">
              <div class="hstat" v-for="s in recordStats(record)" :key="s.label">
...
onMounted(() => { after.setHistory(mockHistory) })
...
function recordStats(r: typeof history[0]) {
  return [
    { icon: 'straighten',    color: 'primary',   val: `${r.distanceKm} km`,        label: '距離' },
    { icon: 'schedule',      color: 'primary',   val: formatDuration(r.durationMin), label: '時間' },
    { icon: 'terrain',       color: 'warning',   val: `+${r.elevationGain}m`,      label: '爬升' },
    { icon: 'local_fire_department', color: 'negative', val: `${r.calories}`,      label: 'kcal' },
  ]
}
...
function difficultyLabel(d: string) { return diffMap[d as keyof typeof diffMap]?.label ?? d }
function difficultyColor(d: string) { return diffMap[d as keyof typeof diffMap]?.color ?? 'grey' }
</script>
...
.diff-easy   { background: rgba(46,204,113,0.11); }
.diff-medium { background: rgba(230,126,34,0.11); }
...
.dbadge-hard   { background: rgba(200,144,42,0.14); color: #A07020; }
.dbadge-expert { background: rgba(217,48,37,0.11); color: #C0392B; }

// ── Detail section ────────────────────────────────────────
.hcard-detail { padding: 0 16px 16px; }
.hcard-divider {
  height: 1px; background: rgba(0,0,0,0.06);
  margin: 0 -16px 12px;
}
:global(.body--dark) .hcard-divider { background: rgba(255,255,255,0.06); }
...
// ── Transition ────────────────────────────────────────────
.expand-fade-enter-active { transition: all 0.3s cubic-bezier(0.34, 1.1, 0.64, 1); }
.expand-fade-leave-active { transition: all 0.2s ease; }
.expand-fade-enter-from   { opacity: 0; transform: translateY(-8px); }
.expand-fade-leave-to     { opacity: 0; transform: translateY(-4px); }
</style>
