<template>
  <q-page class="hike-status-bg" style="min-height: 100dvh">
    ...
    <div class="row items-center q-pa-md q-pt-safe">
      ...
      <q-space />
      <q-btn flat round icon="record_voice_over" color="white" @click="speakStatus">
        <q-tooltip class="bg-dark">語音播報</q-tooltip>
      </q-btn>
      <q-btn flat round icon="map" color="white" @click="$router.push('/hike/map')" />
    </div>
    ...
    <div class="q-px-md q-pb-xl">
      ...
      <div class="weather-bar row items-center q-pa-sm q-mb-md rounded-borders">
        ...
        <span class="text-white text-weight-medium">{{ conditionLabel[hikeStore.liveWeather.condition] }}</span>
        <span class="text-white opacity-80 q-ml-sm">{{ hikeStore.liveWeather.temp }}°C</span>
        <q-space />
        <q-icon name="air" color="white" size="16px" class="q-mr-xs" />
        <span class="text-white text-caption">{{ hikeStore.liveWeather.windSpeed }}km/h</span>
        <q-badge
          class="q-ml-sm"
      ...
      <div class="row q-gutter-sm q-mb-md">
        <BiometricCard
          icon="monitor_heart" label="心率"
          :value="biometrics?.heartRate ?? '--'" unit="bpm"
          :alert="(biometrics?.heartRate ?? 0) > 130" class="col"
        />
        <BiometricCard
          icon="directions_walk" label="步數"
          :value="biometrics?.steps?.toLocaleString() ?? '--'" unit="步" class="col"
        />
        <BiometricCard
          icon="terrain" label="海拔"
          :value="biometrics?.altitude ?? '--'" unit="m" class="col"
        />
      </div>
      ...
      <q-card class="fatigue-card q-mb-md">
        <q-card-section>
          <div class="row items-center q-mb-sm">
            <q-icon name="battery_alert" :color="fatigueColor" size="24px" class="q-mr-sm" />
            <div class="text-subtitle1 text-weight-bold" :class="`text-${fatigueColor}`">
          ...
          <!-- 速度趨勢 -->
          <div class="q-mt-md">
            <div class="text-caption text-grey-6 q-mb-xs">速度趨勢（最近 6 筆）</div>
            <div class="speed-trend row items-end q-gutter-xs">
              <div
                ...
                :class="i === hikeStore.speedHistory.length - 1 ? 'speed-bar-current' : ''"
              />
            </div>
          ...
          <q-banner v-if="hikeStore.fatigueIndex > 60" class="bg-negative text-white rounded-borders q-mt-sm" rounded>
      ...
      <q-card class="ai-panel q-mb-md">
        <q-card-section>
          <div class="row items-center q-mb-md">
            ...
            <div class="q-ml-md">
              <div class="text-h6 text-weight-bold" :class="`text-${actionConfig.color}`">
                {{ actionConfig.label }}
              </div>
              <div class="text-caption text-grey-6">信心度 {{ aiDecision?.confidence }}%</div>
            </div>
            <q-space />
            <q-circular-progress
              :value="aiDecision?.confidence ?? 0"
              size="48px" :color="actionConfig.color"
          ...
          <q-linear-progress
            ...
            :color="riskColor" track-color="grey-3"
            rounded size="8px" class="q-mb-xs"
          />
          <div class="row justify-between text-caption text-grey-6 q-mb-sm">
            <span>安全等級評分</span>
            <span :class="`text-${riskColor}`">{{ aiDecision?.riskScore ?? 0 }}/100</span>
          </div>

          <div class="ai-reason text-body2 text-grey-8">{{ aiDecision?.reason }}</div>
        </q-card-section>
      </q-card>
      ...
      <q-card class="family-banner q-mb-md">
        <q-card-section class="row items-center q-pa-md">
          <q-icon name="family_restroom" color="primary" size="24px" class="q-mr-md" />
          <div class="col">
            <div class="text-subtitle2 text-weight-bold">家人守護模式</div>
            <div class="text-caption text-grey-6">
              {{ familyWatch ? '位置分享已開啟，家人可即時追蹤' : '開啟後家人可即時看到您的位置' }}
            </div>
          </div>
          <q-toggle v-model="familyWatch" color="primary" @update:model-value="hikeStore.toggleFamilyWatch" />
        </q-card-section>
        <q-slide-transition>
          <div v-if="familyWatch" class="q-px-md q-pb-sm">
            <q-linear-progress indeterminate color="primary" size="2px" />
            <div class="text-caption text-primary q-mt-xs">正在傳送位置給 3 位家人...</div>
          </div>
        </q-slide-transition>
      </q-card>
      ...
      <q-card class="hiking-card q-mb-md">
        <q-card-section class="row q-gutter-md text-center">
          ...
          <div class="col">
            ...
            <div class="text-h6 text-weight-bold">{{ hikeStore.deviationCount }}</div>
            <div class="text-caption text-grey-6">偏離次數</div>
          </div>
          <div class="col">
            <q-icon name="notifications" color="negative" />
            <div class="text-h6 text-weight-bold">{{ hikeStore.alertCount }}</div>
      ...
      <!-- SOS 快捷 -->
      <q-btn unelevated size="lg" color="negative" icon="sos" label="緊急求救"
             class="full-width" @click="$router.push('/hike/sos')" />
    </div>
  </q-page>
</template>
...
const weatherIcon = computed(() => conditionIcon[hikeStore.liveWeather.condition] ?? 'wb_sunny')
const weatherRiskColor = computed(() => ({ low: 'positive', medium: 'warning', high: 'negative' }[hikeStore.liveWeather.riskLevel]))
const weatherRiskLabel = computed(() => ({ low: '低風險', medium: '中風險', high: '高風險' }[hikeStore.liveWeather.riskLevel]))
...
const actionConfig = computed(() => actionConfigs[aiDecision.value?.action ?? 'rest'])
