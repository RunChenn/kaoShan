  ...
  <q-page class="hike-map-page">
    ...
    <div class="top-bar">
      ...
      <div class="top-right-controls">
        <q-btn
          ...
          size="sm"
          class="map-fab q-mr-sm"
          @click="toggleOffline"
        >
          <q-tooltip>{{
            hikeStore.isOfflineMode ? '離線模式' : '有訊號'
          }}</q-tooltip>
        </q-btn>
        <q-btn
          dense
    ...
    <transition name="banner-slide">
      <div v-if="hikeStore.deviationAlert" class="map-banner banner-danger">
        ...
        <span class="text-white text-weight-bold" style="font-size: 0.85rem"
          >偏離路線警告！請確認位置</span
        >
        <q-btn
          flat
    ...
    <transition name="banner-slide">
      <div
        v-if="hikeStore.weatherAlert"
        class="map-banner banner-info"
        style="top: 48px"
      >
        <q-icon :name="weatherAlertIcon" color="white" size="18px" />
        <span class="text-white text-weight-bold" style="font-size: 0.85rem">{{
          hikeStore.weatherAlert.message
        }}</span>
        <q-btn
          flat
          dense
    ...
    <transition name="banner-slide">
      <div v-if="hikeStore.isOfflineMode" class="map-banner banner-offline">
        ...
        <span class="text-white" style="font-size: 0.8rem; font-weight: 600"
          >離線模式 · 地圖快取中運作</span
        >
      </div>
    </transition>
    ...
    <q-bottom-sheet v-model="showSheet" seamless>
      <q-card class="sheet-card">
        ...
        <q-card-section class="q-py-sm q-px-lg">
          <div class="sheet-row">
            <div class="sheet-row-icon ring-danger">
              <q-icon name="monitor_heart" color="negative" size="18px" />
            </div>
          ...
          <div class="sheet-row">
            <div class="sheet-row-icon ring-primary">
              <q-icon name="directions_walk" color="primary" size="18px" />
            </div>
...
const weatherIcon = computed(
  () => condIcons[hikeStore.liveWeather.condition] ?? 'wb_sunny',
);
...
onMounted(async () => {
  ...
  weatherTimer = setTimeout(() => {
    hikeStore.setWeatherAlert({
      type: 'rain',
      message: '預計 30 分鐘後降雨，請注意路面濕滑',
    });
  }, 14000);
  setTimeout(() => {
    hikeStore.updateWeather({
      condition: 'cloudy',
      riskLevel: 'medium',
      temp: 14,
    });
...
function centerMap() {
  ...
  if (pos && map.value)
    map.value.flyTo({ center: [pos.lng, pos.lat], zoom: 14 });
}
function zoomIn() {
  map.value?.zoomIn();
}
...
.chip-warn {
  background: rgba(200, 144, 42, 0.88) !important;
  border-color: transparent !important;
}
.chip-danger {
  background: rgba(190, 30, 30, 0.88) !important;
  border-color: transparent !important;
...
:global(.body--dark) .sheet-divider {
