  ...
  >
    ...
    >
      <div style="display: flex; align-items: center; gap: 6px">
        ...
        <span style="color: oklch(68% 0.18 148); font-weight: 700; font-size: 12px; letter-spacing: -0.01em">
          Tweaks Panel
        </span>
      </div>
      <button
        @click="collapsed = !collapsed"
        style="background: none; border: none; color: rgba(255,255,255,0.35); cursor: pointer; font-size: 13px; padding: 0 2px"
      >{{ collapsed ? '▲' : '▼' }}</button>
    </div>
    ...
    <div v-if="!collapsed" style="padding: 12px 14px 14px">
      <PanelRow label="主題模式">
        <SegControl :options="['dark', 'light']" :value="config.theme" @change="v => update({ theme: v as 'dark'|'light' })" />
      </PanelRow>
      <PanelRow label="經驗等級">
        <SegControl :options="['beginner', 'experienced']" :value="config.experience" @change="v => update({ experience: v as 'beginner'|'experienced' })" />
      </PanelRow>
      <PanelRow label="訊號模式">
        <SegControl
          :options="['online', 'offline']" :value="config.signal"
          :colorMap="{ online: 'oklch(62% 0.16 145)', offline: 'oklch(68% 0.18 72)' }"
          @change="v => update({ signal: v as 'online'|'offline' })"
        />
      </PanelRow>
      <PanelRow label="風險等級">
        <SegControl
          :options="['low', 'mid', 'high']" :value="config.risk"
          :colorMap="{ low: 'oklch(62% 0.16 145)', mid: 'oklch(68% 0.18 72)', high: 'oklch(60% 0.22 28)' }"
          @change="v => update({ risk: v as 'low'|'mid'|'high' })"
        />
      </PanelRow>

      <!-- Status readout -->
      <div :style="{ marginTop: '4px', padding: '8px 10px', background: 'rgba(255,255,255,0.04)', borderRadius: '10px', fontSize: '11px', color: 'rgba(255,255,255,0.3)', lineHeight: 1.7 }">
        <div>主題 <span style="color: rgba(255,255,255,0.65); font-weight: 600">{{ config.theme }}</span></div>
        <div>訊號 <span :style="{ color: config.signal === 'offline' ? 'oklch(68% 0.18 72)' : 'oklch(62% 0.16 145)', fontWeight: 600 }">{{ config.signal }}</span></div>
        <div>風險 <span :style="{ color: riskColors[config.risk], fontWeight: 600 }">{{ config.risk }}</span></div>
      </div>
    </div>
  </div>
</template>
...
function handleMouseDown(e: MouseEvent) {
  dragging.value = true
  dragRef.value = { ox: e.clientX, oy: e.clientY, sx: pos.value.x, sy: pos.value.y }
